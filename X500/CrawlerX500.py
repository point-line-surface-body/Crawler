...
#
#URL
#http://directory.app.alcatel-lucent.com/Rupi=CV00XXXXX


import os, sys, re, random
sys.path.append( os.path.dirname( os.path.abspath(__file__) ) )
import urllib, urllib2, pprint, copy, optparse
import pymongo
...
class X500Crawler(object):
	def __init__(self, start_upi = 'CV0010000',home_url='http://directory.app.alcatel-lucent.com'):
		#MongoDB
		...
		#self.url_startpoint = '%s/Rupi=%s' % (self.url_root, start_upi)  #'http://directory.app.alcatel-lucent.com/Rupi=CV0010000'
	...
	def get_html_content(self, url):
		...
		#Proxy
		#135.251.33.16 : 8080 / 135.245.48.13 ：8000 / 135.251.33.31 : 8080 / 135.251.33.15 : 8080 / 135.245.48.13   8000
		proxy_handler = urllib2.ProxyHandler( {"http" : 'http://%s:%s@135.251.33.15:8080' % ( 'XXXX', 'XXXXX') } )
		opener = urllib2.build_opener(proxy_handler,urllib2.HTTPSHandler(debuglevel=0))
		urllib2.install_opener(opener)
		authheader =  None#"Basic %s" % base64.encodestring('%s:%s' % ('svn', 'svn'))[:-1]
		data=None
	...
	def parse_html_content(self, html_content, current_upi):
		...
		for div in divs:
			...
			#OnNET|Company|CSLogin|Email|UPI|Cost|User|Building|Office|Department|Organization|family|title
				...
				if "OnNET" in person_attr_name:
					...
					current_upi_info["Department"]  = str(person_attr_value)
					...
					current_upi_info["BusinessTitle"]  = str(person_attr_value)
		...
		#<li class="tree"><a href="/en/Rupi=CV0010000">XXXX</a></li>
	...
	def check_save_upi( self, current_upi ):
		...
		#URL
		current_upi_url         = '%s/Rupi=%s' % (self.url_root, current_upi)  #'http://directory.app.alcatel-lucent.com/Rupi=CV0020000'
	...
	def run(self):
		while( len(self.queue_uncheck) ):
			...
			current_upi = self.queue_uncheck.pop()
	...
	def test_mongodb(self):
		...
		#	print item
		return recs.count()
		#rec_finded    = self.collect.find_one({"UPI": 'CV0010000'})
...
if __name__ == '__main__':
	...
	t = X500Crawler(options.startupi) #t = X500Crawler('CV0010000')
